...
const { isTrue, isFlase } = require("../../common/utils/functions");
...
class OptionService {
    ...
    async create(optionDto) {
        ...
        if (isFlase(optionDto?.required)) optionDto.required = false;
    ...
    async update(id, optionDto) {
        ...
        if (isTrue(optionDto?.required)) optionDto.required = true;
        else if (isFlase(optionDto?.required)) optionDto.required = false;
    ...
    async findByCategoryId(category) {
        return await this.#model.findOne({ category }, { __V: -1 }).populate([{ path: "category", select: { name: 1, slug: 1 } }]);
    }
    ...
    async findByCategorySlug(slug) {
        const options = await this.#model.aggregate([
            {
                $lookup: {
                    // category collection saved on db with categories name!!
                    from: "categories",
                    localField: "category",
                    foreignField: "_id",
                    as: "category"
